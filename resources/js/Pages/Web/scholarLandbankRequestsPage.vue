...
const rows = computed(() => page.props.landbankRequests ?? {});
...
const loadPage = (pageNumber = 1) => {
    router.get(route("scholar-landbank-requests"), requestData(pageNumber), {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const openRequest = (row) => {
    ...
    router.reload({
        ...
        only: ["details", "landbankRequest"],
        preserveScroll: true,
        onSuccess: () => {
            landbankDialog.value = true;
        },
    });
};
